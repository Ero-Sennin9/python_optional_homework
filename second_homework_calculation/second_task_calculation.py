...
def plot_results():
    ...
    plt.title(f"Сравнение методов вычислений (Нахождение делителей большого числа)", fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig("benchmark_calculation_results.png")
    plt.show()


if __name__ == "__main__":
    number = random.randint(10 ** 16, 10 ** 17)
    print("Число для нахождения делителей: ", number)
    test_methods(number)
    plot_results()